 ...
 * - Copies hand-maintained CSS from 'static/css' into 'dist/static'
...
import { PATHS, CSS, STATIC_CSS, WEB_THEME_DEFINITIONS } from '../src/config/constants.js';
...
async function transformCSSFiles() {
  try {
    ...
    // Copy hand-maintained CSS into dist and queue it for minification
    filesToMinify.push(...await copyStaticCSSFiles());
...
/**
 * Copies hand-maintained CSS files from STATIC_CSS.SRC_DIR into STATIC_CSS.OUT_DIR
 * verbatim. These files are authored by hand rather than generated by Style
 * Dictionary, so they are only relocated here; minification happens in the same
 * batch as the generated output.
 *
 * @returns {Promise<string[]>} Paths of the copied files, for the minification queue
 */
async function copyStaticCSSFiles() {
  /** @type {string[]} */
  const copied = [];
  ...
  let entries;
  try {
    entries = await fs.readdir(STATIC_CSS.SRC_DIR, { withFileTypes: true });
  } catch (error) {
    if (error instanceof Error && /** @type {{code?: string}} */(error).code === 'ENOENT') {
      console.log(`No static CSS directory found at ${STATIC_CSS.SRC_DIR}`);
      return copied;
    }
    throw error;
  }
  ...
  if (sourceFiles.length === 0) {
    console.log(`No static CSS files found in ${STATIC_CSS.SRC_DIR}`);
    return copied;
  }

  await fs.mkdir(STATIC_CSS.OUT_DIR, { recursive: true });

  for (const entry of sourceFiles) {
    const destPath = path.join(STATIC_CSS.OUT_DIR, entry.name);
    await fs.copyFile(path.join(STATIC_CSS.SRC_DIR, entry.name), destPath);
    copied.push(destPath);
    console.log(`Copied static CSS to ${destPath}`);
  }
