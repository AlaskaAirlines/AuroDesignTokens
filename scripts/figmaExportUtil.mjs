...
export default class ExportUtil {
  ...
  static fetchImportTokens(fileName, platform) {
    const figmaImportDir = path.join(process.cwd(), 'node_modules', '@aurodesignsystem', 'auro-tokendefinitions', 'tokens', 'figmaExports');
    const manualImportDir = path.join(process.cwd(), 'node_modules', '@aurodesignsystem', 'auro-tokendefinitions', 'tokens', 'manualTokens', platform);
    const exportJson = this.fetchFile(figmaImportDir, fileName);
    const basicTokens = {
      "basic": {
        "color": {
          ['brand']: JSON.parse(exportJson)['Brand'],
          ['texticon']: JSON.parse(exportJson)['Text & Icon'],
          ['page-background']: JSON.parse(exportJson)['Page Background'],
          ['surface']: JSON.parse(exportJson)['Surface'],
          ['border']: JSON.parse(exportJson)['Border'],
          ['status']: JSON.parse(exportJson)['Status'],
          ['tier-program']: JSON.parse(exportJson)['Tier Program'],
          ['fare-brand']: JSON.parse(exportJson)['Fare Brand'],
        }
      }
    }
    let importTokens = {};
    ...
    if (platform === 'app') {
      const appAdvancedTokens = {
        "uiTokens":JSON.parse(exportJson).uiTokens
      }
      ...
      const webAdvancedTokens = {
        "advanced": {
          "color": JSON.parse(exportJson).advanced
        }
      }
  ...
  static processValueKey(data) {
    ...
    if (typeof data === 'string') {
      ...
      // if alpha property exists and is a number, append corresponding hex alpha
      if (data.hasOwnProperty('alpha') && typeof data.alpha === 'number') {
    ...
    for (const i in obj) {
      if (obj.hasOwnProperty(i)) {
        ...
        if (typeof obj[i] === 'object' && obj[i] !== null) {
          keys = keys.concat(this.processAllValueKeys(obj[i]));
        }
      }
    }
  }
  ...
  static processAllTokenRefs() {
    ...
    if (allValueKeys > 0) {
      ...
      this.processAllTokenRefs(this.processedJson, tokensToReplace)
    }
  }
  ...
  static writeToFile(fileName, platform, data) {
    try {
      ...
      if (!fs.existsSync(outDir)) {
        fs.mkdir(path.join(process.cwd(), outDir), (err) => {
          if (err) {
      ...
      if (platform) {
        if (!fs.existsSync(path.join(process.cwd(), outDir, platform))) {
          fs.mkdir(path.join(process.cwd(), outDir, platform), (err) => {
            if (err) {
              console.error('Unable to create output directory:', err);
            }
          });
